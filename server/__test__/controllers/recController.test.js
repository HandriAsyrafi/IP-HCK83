...
describe('RecController', () => {
  ...
  describe('POST /recommendations/generate', () => {
    ...
    it('should generate recommendation successfully', async () => {
      ...
      GeminiService.generateWeaponRecommendation.mockResolvedValue(mockAiRecommendation);
  ...
  describe('GET /monsters/:monsterId/analyze', () => {
    it('should analyze monster successfully', async () => {
      ...
      expect(GeminiService.generateMonsterAnalysis).toHaveBeenCalledWith(mockMonster);
    });
  ...
  describe('GET /monsters/:monsterId/best-weapon', () => {
    ...
    it('should return best weapon recommendation', async () => {
      ...
      expect(response.body.recommendation.reasoning).toBe('Water element is super effective');
      expect(response.body.recommendation.effectivenessScore).toBeGreaterThan(0);
    });

    it('should filter weapons by rarity when specified', async () => {
      Monster.findByPk.mockResolvedValue(mockMonster);
      Weapon.findAll.mockResolvedValue(mockWeapons.filter(w => w.rarity === 7));
      
      const response = await request(app).get('/monsters/1/best-weapon?rarity=7');
